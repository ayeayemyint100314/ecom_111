...
if ($_SERVER['REQUEST_METHOD'] == "GET" && $_GET['val'] == 'del') {
  ...
  {  $id = $_POST['id'];
      ...
      {
          try{
              $sql = "update product set product_name=?, cost=?, price=?, 
                        description=?, category=?, quantity=?,
                         image_path=? where id=?";
              $stmt = $conn->prepare($sql);
              $status = $stmt->execute([$productName, $cost, $price,
                       $description,
                        $category, $quantity,$filePath, $id  ]);
              if($status)
              {
                 $_SESSION['message'] = "Update Success on id $id";
                 header("Location:viewInfo.php?show=products");
  ...
  <div class="container-fluid">
    ...
    <div class="row">
      ...
      <div class="col-md-10">
        ...
        <form action="editProduct.php" class="form" method="post" 
          ...
          <div class="row">
            ...
            <div class="col-md-6 py-3">
              <div class="mb-3">
                ...
                <textarea name="desc" id="desc" value="<?php if (isset($product)) {
